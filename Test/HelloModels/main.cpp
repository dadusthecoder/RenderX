...
struct OrbitCamera {
    ...
    float     yaw      = 0.0f;  // was 45 — start directly in front
    float     pitch    = 15.0f; // was 20 — slight downward angle
    float     distance = 15.0f;  // was 10 — closer default, scroll to adjust
    ...
    Rx::Camera toRxCamera() const {
        ...
        // Spherical to Cartesian — positive pitch = above target
        glm::vec3 pos = {target.x + distance * cos(pitchR) * sin(yawR),
                         target.y + distance * sin(pitchR), // Y is up
        ...
        cam.fovY      = 60.0f;
        cam.nearPlane = 0.1f;
        cam.farPlane  = 500.0f; // was 100 — increase for large models
        return cam;
    }
    ...
    void onScroll(double dy) { distance = glm::clamp(distance - (float)dy, 0.5f, 50.0f); }
};
...
int main(int argc, char** argv) {
    //const char* modelPath = argc > 1 ? argv[1] : "Alien/Test_Alien-Animal-Blender_2.81.fbx";
    const char* modelPath = argc > 1 ? argv[1] : "sopnza_palace/Sponza_palace.gltf";
    // default test model
    //: "sphere2.fbx";
    //"Alien/Untitled.fbx"
    //"D:/dev/cpp/RenderX/build/bin/Debug/RenderX/sopnza_palace/Sponza_palace.gltf"
    glfwInit();
