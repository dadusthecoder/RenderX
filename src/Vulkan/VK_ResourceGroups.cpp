...
VkDescriptorPool VulkanDescriptorPoolManager::createPool(uint32_t maxSets, bool allowFree, bool updateAfterBind) {
...
VulkanResourceGroupLayout VulkanDescriptorManager::createLayout(const ResourceGroupLayoutDesc& layout) {
    ...
    for (auto& b : layout.resourcebindings) {
        ...
        vk.stageFlags      = MapShaderStageFlags(b.stages);
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(m_Ctx.device->logical(), &ci, nullptr, &out.layout));

    return out;
}

VulkanResourceGroup VulkanDescriptorManager::createGroup(const ResourceGroupLayoutHandle& handle,
    ...
    if (group.model == DescriptorBindingModel::Dynamic) {
        ...
        VK_CHECK(vkAllocateDescriptorSets(m_Ctx.device->logical(), &ai, &group.set));
        ...
        if (!writes.empty()) {
            vkUpdateDescriptorSets(m_Ctx.device->logical(), (uint32_t)writes.size(), writes.data(), 0, nullptr);
        }
    }
    ...
    // DESCRIPTOR BUFFER PATH
        ...
        bi.size  = totalSize;
        bi.usage = VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;

        VmaAllocationCreateInfo ai{};
        ai.usage = VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE;

        VK_CHECK(vmaCreateBuffer(m_Ctx.allocator->handle(),
                                 &bi,
                                 &ai,
        ...
        group.descriptorBuffer.address = vkGetBufferDeviceAddress(m_Ctx.device->logical(), &addrInfo);
...
void VulkanDescriptorManager::bind(VkCommandBuffer            cmd,
    ...
    case DescriptorBindingModel::Dynamic:
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, setIndex, 1, &group.set, 0, nullptr);
        break;

    case DescriptorBindingModel::DynamicUniform:
        vkCmdBindDescriptorSets(
            cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, setIndex, 1, &group.set, 1, &group.dynamicOffset);
        break;
