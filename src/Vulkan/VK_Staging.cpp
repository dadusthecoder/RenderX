...
namespace RxVK {
	...
	VulkanStagingAllocator::VulkanStagingAllocator(VulkanContext& ctx, uint32_t chunkSize)
		: m_Ctx(ctx), m_ChunkSize(chunkSize) {
	}
	...
	StagingChunk VulkanStagingAllocator::createChunk(uint32_t size) {
		...
		bool success = m_Ctx.allocator->createBuffer(
			size,
...
//TODO
	...
	void VulkanDeferredUploader::uploadBuffer(
		BufferHandle dstBuffer,
		const void* data,
		uint32_t size,
		uint32_t dstOffset) {
	...
	void VulkanDeferredUploader::uploadTexture(
		TextureHandle dstTexture,
		const void* data,
		uint32_t size,
		const TextureCopyRegion& region) {
