...
namespace RxVK {
	...
	inline VkSubresourceLayout GetImageSubresourceLayout(VkImage image,
		VkFormat format,
		uint32_t mipLevel = 0,
	...
	TextureHandle VKCreateTexture(const TextureDesc& desc) {
		...
		imageInfo.extent.height = (desc.type == TextureType::TEXTURE_1D ||
									  desc.type == TextureType::TEXTURE_1D_ARRAY)
									  ? 1
									  : desc.height;
