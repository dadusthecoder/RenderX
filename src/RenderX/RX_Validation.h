...
namespace Validation {
	...
	struct ResourceInfo {
		...
		ResourceInfo() : handleId(0), state(ResourceState::CREATED),
						 creationFrame(0), lastUsedFrame(0) {}
	};
	...
	struct CommandListInfo {
		...
		CommandListInfo() : state(CommandListState::INITIAL),
							isInsideRenderPass(false), isInsideRendering(false),
							boundPipeline(0), boundIndexBuffer(0), boundFramebuffer(0),
							activeRenderPass(0), recordingFrame(0) {}
	};
	...
	struct ValidationMessage {
		...
		ValidationMessage(ValidationSeverity sev, ValidationCategory cat,
			const std::string& msg, const std::string& f = "", int l = 0)
			: severity(sev), category(cat), message(msg), file(f), line(l), frame(0) {}
	};
	...
	public:
		...
		// Message reporting
		void Report(ValidationSeverity severity, ValidationCategory category,
...
#define RX_VALIDATE_BUFFER_UNREGISTER(handle) \
	::Rx::Validation::ValidationLayer::Get().UnregisterBuffer(handle)

#define RX_VALIDATE_BUFFER(handle, context) \
	::Rx::Validation::ValidationLayer::Get().ValidateBuffer(handle, context)

#define RX_VALIDATE_BUFFER_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateBufferDesc(desc)

#define RX_VALIDATE_BUFFER_MAP(handle, ptr) \
	::Rx::Validation::ValidationLayer::Get().OnBufferMap(handle, ptr)

#define RX_VALIDATE_BUFFER_UNMAP(handle) \
	::Rx::Validation::ValidationLayer::Get().OnBufferUnmap(handle)
...
#define RX_VALIDATE_TEXTURE_UNREGISTER(handle) \
	::Rx::Validation::ValidationLayer::Get().UnregisterTexture(handle)

#define RX_VALIDATE_TEXTURE(handle, context) \
	::Rx::Validation::ValidationLayer::Get().ValidateTexture(handle, context)

#define RX_VALIDATE_TEXTURE_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateTextureDesc(desc)
...
#define RX_VALIDATE_PIPELINE_UNREGISTER(handle) \
	::Rx::Validation::ValidationLayer::Get().UnregisterPipeline(handle)

#define RX_VALIDATE_PIPELINE(handle, context) \
	::Rx::Validation::ValidationLayer::Get().ValidatePipeline(handle, context)

#define RX_VALIDATE_PIPELINE_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidatePipelineDesc(desc)
...
#define RX_VALIDATE_RESOURCE_GROUP_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateResourceGroupDesc(desc)

// Command list validation macros
#define RX_VALIDATE_CMD_REGISTER(cmdList) \
	::Rx::Validation::ValidationLayer::Get().RegisterCommandList(cmdList)

#define RX_VALIDATE_CMD_UNREGISTER(cmdList) \
	::Rx::Validation::ValidationLayer::Get().UnregisterCommandList(cmdList)

#define RX_VALIDATE_CMD_BEGIN(cmdList) \
	::Rx::Validation::ValidationLayer::Get().OnCommandListBegin(cmdList)

#define RX_VALIDATE_CMD_END(cmdList) \
	::Rx::Validation::ValidationLayer::Get().OnCommandListEnd(cmdList)

#define RX_VALIDATE_CMD_SUBMIT(cmdList) \
	::Rx::Validation::ValidationLayer::Get().OnCommandListSubmit(cmdList)

#define RX_VALIDATE_CMD_RESET(cmdList) \
	::Rx::Validation::ValidationLayer::Get().OnCommandListReset(cmdList)
...
#define RX_VALIDATE_END_RENDER_PASS(cmdList) \
	::Rx::Validation::ValidationLayer::Get().ValidateEndRenderPass(cmdList)

#define RX_VALIDATE_BEGIN_RENDERING(cmdList, desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateBeginRendering(cmdList, desc)

#define RX_VALIDATE_END_RENDERING(cmdList) \
	::Rx::Validation::ValidationLayer::Get().ValidateEndRendering(cmdList)

#define RX_VALIDATE_RENDER_PASS_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateRenderPassDesc(desc)

#define RX_VALIDATE_FRAMEBUFFER_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateFramebufferDesc(desc)

#define RX_VALIDATE_RENDERING_DESC(desc) \
	::Rx::Validation::ValidationLayer::Get().ValidateRenderingDesc(desc)

// Synchronization validation macros
#define RX_VALIDATE_QUEUE_SUBMIT(queue, info) \
	::Rx::Validation::ValidationLayer::Get().ValidateQueueSubmit(queue, info)

#define RX_VALIDATE_QUEUE_WAIT(queue, value) \
	::Rx::Validation::ValidationLayer::Get().ValidateQueueWait(queue, value)
