...
namespace Rx {
	...
	public:
		explicit RateLimitedSink(
			std::shared_ptr<spdlog::sinks::sink> wrapped_sink,
			std::chrono::milliseconds rate_limit_interval = std::chrono::milliseconds(100))
			: wrapped_sink_(wrapped_sink), rate_limit_interval_(rate_limit_interval) {}
	...
	protected:
		void sink_it_(const spdlog::details::log_msg& msg) override {
			...
			if (it == last_log_time_.end() ||
				(now - it->second) >= rate_limit_interval_) {
				...
				if (suppress_it != suppressed_count_.end() && suppress_it->second > 0) {
					...
					std::string modified_payload = fmt::format("{} (+{} suppressed)",
						msg_key, suppress_it->second);
	...
	void Log::Init() {
		...
		fileSink->set_pattern_for_level(spdlog::level::trace,
			"[%Y-%m-%d %T.%e] [T] [%n] %v");
		fileSink->set_pattern_for_level(spdlog::level::debug,
			"[%Y-%m-%d %T.%e] [D] [%n] [%!] %v");
		fileSink->set_pattern_for_level(spdlog::level::info,
			"[%Y-%m-%d %T.%e] [I] [%n] %v");
		fileSink->set_pattern_for_level(spdlog::level::warn,
			"[%Y-%m-%d %T.%e] [W] [thread %t] [%n] [%!:%#] %v");
		fileSink->set_pattern_for_level(spdlog::level::err,
			"[%Y-%m-%d %T.%e] [E] [thread %t] [%n] [%s:%#] [%!] %v");
		fileSink->set_pattern_for_level(spdlog::level::critical,
			"[%Y-%m-%d %T.%e] [C] [thread %t] [%n] [%s:%#] [%!] %v");
