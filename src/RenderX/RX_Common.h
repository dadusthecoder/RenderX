...
// Describes a single binding slot in a
// ResourceGroupLayout
struct ResourceGroupLayoutItem {
    ...
    // Array size (1 for non-arrays, >1 for
    // arrays) For Bindless indexing
    uint32_t count;

    ResourceGroupLayoutItem()
        : binding(0),
          type(ResourceType::CONSTANT_BUFFER),
          stages(PipelineStage::ALL_GRAPHICS),
          count(1) {}

    ResourceGroupLayoutItem(uint32_t      bind,
                            ResourceType  t,
                            PipelineStage s   = PipelineStage::ALL_GRAPHICS,
                            uint32_t      cnt = 1)
        : binding(bind),
          type(t),
          stages(s),
    ...
    static ResourceGroupLayoutItem ConstantBuffer(uint32_t      binding,
    ...
    StorageBuffer(uint32_t binding, PipelineStage stages = PipelineStage::ALL_GRAPHICS, bool writable = false) {
        return ResourceGroupLayoutItem(
            binding, writable ? ResourceType::RW_STORAGE_BUFFER : ResourceType::STORAGE_BUFFER, stages, 1);
    }

    static ResourceGroupLayoutItem
    Texture_SRV(uint32_t binding, PipelineStage stages = PipelineStage::ALL_GRAPHICS, uint32_t count = 1) {
        return ResourceGroupLayoutItem(binding, ResourceType::TEXTURE_SRV, stages, count);
    }

    static ResourceGroupLayoutItem
    Texture_UAV(uint32_t binding, PipelineStage stages = PipelineStage::ALL_GRAPHICS, uint32_t count = 1) {
        return ResourceGroupLayoutItem(binding, ResourceType::TEXTURE_UAV, stages, count);
    }

    static ResourceGroupLayoutItem Sampler(uint32_t binding, PipelineStage stages = PipelineStage::ALL_GRAPHICS) {
...
/// Describes the layout/structure of a pipeline
/// inputs
...
/// Describes a single resource binding in a
/// descriptor set instance
struct ResourceGroupItem {
    uint32_t     binding;
    ResourceType type;
    uint32_t     arrayIndex = 0; // For Bindless Resource to arrays of
                                 // descriptors
