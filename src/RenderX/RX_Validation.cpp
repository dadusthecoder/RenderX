...
namespace Validation {
	...
	void ValidationLayer::Shutdown() {
		...
		if (!buffers_.empty()) {
			...
			for (const auto& [id, info] : buffers_) {
				RX_CORE_ERROR("  - Buffer 0x{:016X} ({})", id,
		...
		if (!textures_.empty()) {
			...
			for (const auto& [id, info] : textures_) {
				RX_CORE_ERROR("  - Texture 0x{:016X} ({})", id,
					info.debugName.empty() ? "unnamed" : info.debugName);
			}
		}
		...
		RX_CORE_INFO("Validation Layer shutdown - Total _ERRORs: {}, warnings: {}",
			errorCount_, warningCount_);
	...
	void ValidationLayer::Report(ValidationSeverity severity, ValidationCategory category,
	...
	void ValidationLayer::RegisterBuffer(BufferHandle handle, const BufferDesc& desc, const char* debugName) {
		if (!IsCategoryEnabled(ValidationCategory::RESOURCE)) return;
	...
	void ValidationLayer::UnregisterBuffer(BufferHandle handle) {
		...
		if (it == buffers_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Attempting to destroy non-existent buffer 0x{:016X}", handle.id));
			return;
		}

		if (it->second.isMapped) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
	...
	bool ValidationLayer::ValidateBuffer(BufferHandle handle, const char* context) {
		if (!IsCategoryEnabled(ValidationCategory::HANDLE)) return true;
		...
		if (it == buffers_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Buffer 0x{:016X} not found ({})", handle.id,
		...
		if (it->second.state == ResourceState::DESTROYED) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
				fmt::format("Using destroyed buffer 0x{:016X} ({})", handle.id,
					context ? context : "unknown context"));
			return false;
		}
	...
	void ValidationLayer::ValidateBufferDesc(const BufferDesc& desc) {
		...
		if (!IsValidBufferFlags(desc.usage)) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
				"Invalid buffer usage flags combination");
		}
		...
		if (desc.memoryType == MemoryType::GPU_ONLY && hasDynamic) {
			Report(ValidationSeverity::WARNING, ValidationCategory::RESOURCE,
				"GPU_ONLY memory with DYNAMIC usage may cause performance issues");
		}

		if (desc.memoryType == MemoryType::CPU_TO_GPU &&
			!Has(desc.usage, BufferUsage::UNIFORM) &&
	...
	void ValidationLayer::OnBufferMap(BufferHandle handle, void* ptr) {
		if (!IsCategoryEnabled(ValidationCategory::MEMORY)) return;
		...
		if (it->second.isMapped) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::MEMORY,
	...
	void ValidationLayer::OnBufferUnmap(BufferHandle handle) {
		...
		if (!it->second.isMapped) {
			Report(ValidationSeverity::WARNING, ValidationCategory::MEMORY,
	...
	void ValidationLayer::UnregisterTexture(TextureHandle handle) {
		...
		if (!it->second.viewHandles.empty()) {
			Report(ValidationSeverity::WARNING, ValidationCategory::RESOURCE,
				fmt::format("Texture 0x{:016X} destroyed with {} active views",
					handle.id, it->second.viewHandles.size()));
		}
	...
	bool ValidationLayer::ValidateTexture(TextureHandle handle, const char* context) {
		...
		if (it == textures_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Texture 0x{:016X} not found ({})", handle.id,
					context ? context : "unknown context"));
			return false;
		}
	...
	void ValidationLayer::ValidateTextureDesc(const TextureDesc& desc) {
		...
		if (desc.width == 0 || desc.height == 0) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
				"Texture dimensions cannot be 0");
		}
		...
		if (desc.arrayLayers == 0) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
				"Texture must have at least 1 array layer");
		}
		...
		bool isDepthStencil = desc.format == Format::D24_UNORM_S8_UINT ||
	...
	bool ValidationLayer::ValidateTextureView(TextureViewHandle handle, const char* context) {
		...
		if (it == textureViews_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Texture view 0x{:016X} not found ({})", handle.id,
					context ? context : "unknown context"));
			return false;
		}
	...
	void ValidationLayer::RegisterPipeline(PipelineHandle handle, const PipelineDesc& desc, const char* debugName) {
		if (!IsCategoryEnabled(ValidationCategory::PIPELINE)) return;
	...
	bool ValidationLayer::ValidatePipeline(PipelineHandle handle, const char* context) {
		...
		if (it == pipelines_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Pipeline 0x{:016X} not found ({})", handle.id,
					context ? context : "unknown context"));
			return false;
		}
	...
	void ValidationLayer::ValidatePipelineDesc(const PipelineDesc& desc) {
		...
		if (desc.shaders.empty()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::PIPELINE,
				"Pipeline must have at least one shader");
			return;
		}
		...
		for (const auto& shader : desc.shaders) {
			if (!shader.IsValid()) {
				Report(ValidationSeverity::_ERROR, ValidationCategory::PIPELINE,
					"Pipeline contains invalid shader handle");
			}
		}

		if (!desc.layout.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::PIPELINE,
				"Pipeline has invalid layout handle");
		}

		// Color format validation
		if (desc.colorFromats.empty() && !desc.renderPass.IsValid()) {
			Report(ValidationSeverity::WARNING, ValidationCategory::PIPELINE,
	...
	void ValidationLayer::RegisterResourceGroup(ResourceGroupHandle handle, const ResourceGroupDesc& desc) {
		if (!IsCategoryEnabled(ValidationCategory::DESCRIPTOR)) return;
	...
	bool ValidationLayer::ValidateResourceGroup(ResourceGroupHandle handle, const char* context) {
		...
		if (it == resourceGroups_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				fmt::format("Resource group 0x{:016X} not found ({})", handle.id,
					context ? context : "unknown context"));
			return false;
		}
	...
	void ValidationLayer::ValidateResourceGroupDesc(const ResourceGroupDesc& desc) {
		...
		if (!desc.pipelinelayout.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::DESCRIPTOR,
				"Resource group has invalid pipeline layout");
		}

		if (!desc.layout.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::DESCRIPTOR,
				"Resource group has invalid layout");
		}
		...
		for (const auto& resource : desc.Resources) {
			...
			case ResourceType::COMBINED_TEXTURE_SAMPLER:
				if (!resource.combinedHandles.texture.IsValid() ||
					!resource.combinedHandles.sampler.IsValid()) {
					Report(ValidationSeverity::_ERROR, ValidationCategory::DESCRIPTOR,
	...
	void ValidationLayer::RegisterCommandList(CommandList* cmdList) {
		if (!IsCategoryEnabled(ValidationCategory::COMMAND_LIST)) return;
	...
	void ValidationLayer::UnregisterCommandList(CommandList* cmdList) {
		...
		if (it == commandLists_.end()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::COMMAND_LIST,
	...
	void ValidationLayer::OnCommandListBegin(CommandList* cmdList) {
		...
		if (info->state == CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"Command list already in recording state");
			return;
		}
	...
	void ValidationLayer::OnCommandListEnd(CommandList* cmdList) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				fmt::format("Command list not in recording state (current: {})",
					CommandListStateToString(info->state)));
			return;
		}

		if (info->isInsideRenderPass) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RENDER_PASS,
				"Command list ended while inside render pass");
		}
	...
	void ValidationLayer::OnCommandListSubmit(CommandList* cmdList) {
		...
		if (info->state != CommandListState::EXECUTABLE) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				fmt::format("Cannot submit command list in state: {}",
					CommandListStateToString(info->state)));
			return;
		}
	...
	void ValidationLayer::ValidateDrawCall(CommandList* cmdList, uint32_t vertexCount, uint32_t instanceCount) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"Draw call outside recording state");
			return;
		}
		...
		if (!info->boundPipeline.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::PIPELINE,
				"Draw call without bound pipeline");
		}

		if (info->boundVertexBuffers.empty()) {
			Report(ValidationSeverity::WARNING, ValidationCategory::STATE,
				"Draw call without bound vertex buffer");
		}

		if (vertexCount == 0) {
			Report(ValidationSeverity::WARNING, ValidationCategory::COMMAND_LIST,
				"Draw call with 0 vertices");
		}

		if (instanceCount == 0) {
			Report(ValidationSeverity::WARNING, ValidationCategory::COMMAND_LIST,
				"Draw call with 0 instances");
		}
	}
	...
	void ValidationLayer::ValidateDrawIndexed(CommandList* cmdList, uint32_t indexCount, uint32_t instanceCount) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"Draw indexed call outside recording state");
			return;
		}
		...
		if (!info->boundPipeline.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::PIPELINE,
				"Draw indexed call without bound pipeline");
		}

		if (!info->boundIndexBuffer.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"Draw indexed call without bound index buffer");
		}

		if (info->boundVertexBuffers.empty()) {
			Report(ValidationSeverity::WARNING, ValidationCategory::STATE,
				"Draw indexed call without bound vertex buffer");
		}

		if (indexCount == 0) {
			Report(ValidationSeverity::WARNING, ValidationCategory::COMMAND_LIST,
				"Draw indexed call with 0 indices");
		}
	}

	void ValidationLayer::ValidateBeginRenderPass(CommandList* cmdList, RenderPassHandle pass) {
		if (!IsCategoryEnabled(ValidationCategory::RENDER_PASS)) return;
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"BeginRenderPass outside recording state");
			return;
		}
		...
		if (!pass.IsValid()) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::HANDLE,
				"BeginRenderPass with invalid render pass handle");
			return;
		}
	...
	void ValidationLayer::ValidateEndRenderPass(CommandList* cmdList) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"EndRenderPass outside recording state");
			return;
		}
	...
	void ValidationLayer::ValidateBeginRendering(CommandList* cmdList, const RenderingDesc& desc) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"BeginRendering outside recording state");
			return;
		}
		...
		if (desc.colorAttachments.empty() && !desc.hasDepthStencil) {
			Report(ValidationSeverity::WARNING, ValidationCategory::RENDER_PASS,
	...
	void ValidationLayer::ValidateEndRendering(CommandList* cmdList) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"EndRendering outside recording state");
			return;
		}
	...
	void ValidationLayer::ValidateSetPipeline(CommandList* cmdList, PipelineHandle pipeline) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"SetPipeline outside recording state");
			return;
		}
	...
	void ValidationLayer::ValidateSetVertexBuffer(CommandList* cmdList, BufferHandle buffer) {
		if (!IsCategoryEnabled(ValidationCategory::STATE)) return;
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"SetVertexBuffer outside recording state");
			return;
		}
	...
	void ValidationLayer::ValidateSetIndexBuffer(CommandList* cmdList, BufferHandle buffer) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"SetIndexBuffer outside recording state");
			return;
		}
	...
	void ValidationLayer::ValidateSetResourceGroup(CommandList* cmdList, ResourceGroupHandle group) {
		...
		if (info->state != CommandListState::RECORDING) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::STATE,
				"SetResourceGroup outside recording state");
			return;
		}
			...
			if (region.srcOffset + region.size > srcIt->second.desc.size) {
				Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
					fmt::format("Buffer copy source out of bounds (offset: {}, size: {}, buffer size: {})",
						region.srcOffset, region.size, srcIt->second.desc.size));
			}
			...
			if (region.dstOffset + region.size > dstIt->second.desc.size) {
				Report(ValidationSeverity::_ERROR, ValidationCategory::RESOURCE,
					fmt::format("Buffer copy destination out of bounds (offset: {}, size: {}, buffer size: {})",
						region.dstOffset, region.size, dstIt->second.desc.size));
			}
		}
	}
			...
			if (offset + size > it->second.desc.size) {
				Report(ValidationSeverity::_ERROR, ValidationCategory::MEMORY,
					fmt::format("Buffer write out of bounds (offset: {}, size: {}, buffer size: {})",
						offset, size, it->second.desc.size));
			}
	...
	void ValidationLayer::ValidateFramebufferDesc(const FramebufferDesc& desc) {
		...
		if (desc.width == 0 || desc.height == 0) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::RENDER_PASS,
				"Framebuffer dimensions cannot be 0");
		}
	...
	void ValidationLayer::ValidateQueueSubmit(QueueType queue, const SubmitInfo& info) {
		if (!IsCategoryEnabled(ValidationCategory::SYNCHRONIZATION)) return;

		if (!info.commandList) {
			Report(ValidationSeverity::_ERROR, ValidationCategory::SYNCHRONIZATION,
				"Queue submit with null command list");
			return;
		}

		if (info.commandListCount == 0) {
			Report(ValidationSeverity::WARNING, ValidationCategory::SYNCHRONIZATION,
				"Queue submit with 0 command list count");
		}
	}
	...
	void ValidationLayer::LogMessage(const ValidationMessage& msg) {
		...
		if (config_.logToConsole) {
			if (!msg.file.empty()) {
				Log::Core()->log(logLevel, "[VALIDATION {}] {} ({}:{})",
					severityStr, msg.message, msg.file, msg.line);
			}
			else {
				Log::Core()->log(logLevel, "[VALIDATION {}] {}",
					severityStr, msg.message);
			}
		}
	}
